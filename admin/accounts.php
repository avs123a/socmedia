...
$users=$mysqli->query("select id,username,email,verified,role,status from users where role!='admin'");
if(isset($_POST['mod']))
{
	$userid=$_POST['userid'];
	$mysqli->query("update users set role='moderator' where id=$userid");
}
if(isset($_POST['ban']))
{
	$userid=$_POST['userid'];
	$mysqli->query("update users set status='banned' where id=$userid");
}
if(isset($_POST['unl']))
{
	$userid=$_POST['userid'];
	$mysqli->query("update users set status='active' where id=$userid");
}
if(isset($_POST['del']))
{
	$userid=$_POST['userid'];
		...
		<table class="col-xs-11">
		 ...
		 <?php  while($row=mysqli_fetch_array($users)){ ?>
		   <tr>
				...
				<td class="col-xs-1"><?php echo $row['role'] ?></td>
				<td class="col-xs-1"><?php echo $row['status'] ?></td>
				<td class="col-xs-1">
				  <form name="update_user" action="http://mysocmedia/admin/accounts.php" method="post">
				     <input type="hidden" name="userid" value="<?= $row['id'] ?>">
				     <input type="submit" name="mod" value="Set Moderator"/>
					 <input type="submit" name="ban" value="Ban"/>
					 <input type="submit" name="unl" value="Unlock"/>
