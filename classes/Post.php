              ...
              <?php  foreach($dbposts as $p) { 
                        ...
                        if (!DB::query('SELECT post_id FROM post_likes WHERE post_id=:postid AND user_id=:userid', array(':postid'=>$p['id'], ':userid'=>$loggedInUserId))) {
                                ...
                                <form action='profile.php?username=$username&postid=".$p['id']."' method='post'>
                                        <input type='submit' class='glyphicon glyphicon-heart' style='color:#eb3b60;background-image:url(&quot;none&quot;);background-color:transparent;' name='like' value='Like'>
                                ...
                                ";
							     ...
							     $date=$dbposts['posted_at'];
                                $posts .= "<img src='".$p['postimg']."'>".self::link_add($p['body'])."
								<footer>Posted by @$username on $date
                                <form action='profile.php?username=$username&postid=".$p['id']."' method='post'>
                                <input type='submit' name='unlike' value='Unlike'>
                                <span>".$p['likes']." likes</span></footer>
                                ";
                                if ($userid == $loggedInUserId) {
                                        $posts .= "<input type='submit' name='deletepost' value='x' />";
                                }
